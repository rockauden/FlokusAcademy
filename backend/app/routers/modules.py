...
async def list_modules(course_id: Optional[int] = None, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_active_user)):
    units = await UnitRepository.list(db, tenant_id=user.tenant_id, program_id=course_id)
