...
# --- UTAH CORE STANDARDS LIBRARY (5TH GRADE) ---
UTAH_5TH_STANDARDS = {
    "Mathematics": [
    ...
    "Science (SEEd)": [
        "5.1 - Characteristics & Interactions of Earth's Systems",
        "5.2 - Properties & Changes of Matter",
        "5.3 - Cycling of Matter in Ecosystems"
    ],
    "Social Studies": [
        "5.1 - Geography & Early Peoples",
        "5.2 - The American Revolution",
        "5.3 - The United States Constitution",
        "5.4 - Westward Expansion & Civil War",
        "5.5 - Modern America & Social Movements",
        "5.6 - Civic Participation & Economics"
    ],
    "Language Arts (ELA)": [
        "RL.5 - Reading: Literature",
        "RI.5 - Reading: Informational Text",
        "W.5 - Writing & Creative Expression",
        "L.5 - Language, Grammar & Mechanics"
    ],
    "Applied STEM & Electives": [
        "CS.5 - Computer Science & Algorithms",
        "PE.5 - Physical Education & Motor Skills",
        "FIN.5 - Financial Literacy"
    ]
}
...
st.sidebar.link_button("🐉 Open Flokus Bestiary", "http://localhost:8502")
            ...
            for pillar in pillars:
                ...
                for _, row in pillar_df.iterrows():
                    ...
                    with st.expander(expander_label):
                        ...
                        # Check if the login_url is a valid string and not empty/null
                        if pd.notna(row['login_url']) and row['login_url']:
                           st.link_button(f"🚀 Launch {row['platform']}", row['login_url'])
                        else:
                           # If there is no URL (like the physical robotics kit), display a disabled button or a label
                           st.button(f"🛠️ {row['platform']} (Offline Activity)", disabled=True)
...
# --- ADMIN VIEW ---
    ...
    with tab1:
        ...
        # 1. Fetch existing units AND the raw subject_name from database
        try:
            units_df = run_query("""
                SELECT u.unit_id, 
                       s.pillar || ' ➔ ' || s.subject_name || ' - ' || u.unit_name as full_name, 
                       u.subject_id,
                       s.subject_name  -- <-- Added this so we can auto-map
                FROM units u
        ...
        if not units_df.empty:
            ...
            # --- AUTO-MAPPING LOGIC ---
            # Extract the raw subject name for the selected unit
            db_subject = units_df.loc[units_df['unit_id'] == unit_selection, 'subject_name'].values[0]
            
            # Map your database subject names directly to your UTAH_5TH_STANDARDS keys
            mapping = {
                "Language Arts": "Language Arts (ELA)",
                "Mathematics": "Mathematics",
                "Science": "Science (SEEd)",
                "Social Studies": "Social Studies",
                "Applied STEM": "Applied STEM & Electives" 
            }
            
            # Find the matching Utah key (default to Electives if it's a weird edge case)
            utah_key = mapping.get(db_subject, "Applied STEM & Electives")
            
            # 3. Dynamic Utah Core Standards Selection (The broad category dropdown is GONE!)
            selected_standard = st.selectbox(
                f"Align with Utah Core Standard ({utah_key})", 
                UTAH_5TH_STANDARDS[utah_key]
            )
        ...
        # 4. Standard Task Inputs
        title = st.text_input("Task Title")
        url = st.text_input("URL Link (Optional)")
        
        # 5. Assignment Execution
        if st.button("Assign Task"):
            if title and unit_selection and selected_standard:
                subj_id = int(units_df.loc[units_df['unit_id'] == unit_selection, 'subject_id'].values[0])
                
                execute_query(
                    "INSERT INTO assignments (subject_id, unit_id, title, url_link, status, standard_tag) VALUES (?, ?, ?, ?, 'pending', ?)",
                    (subj_id, unit_selection, title, url, selected_standard)
                )
                
                st.success(f"Assigned successfully! Tagged with: {selected_standard}")
            else:
                st.warning("Please enter a task title and ensure a unit is selected.")
