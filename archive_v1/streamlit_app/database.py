...
def verify_db_schema():
    ...
    try:
        ...
        # Self-Healing Medium Column (Offline vs Online)
        try:
            cursor.execute("SELECT medium FROM tasks LIMIT 1")
        except sqlite3.OperationalError:
            cursor.execute("ALTER TABLE tasks ADD COLUMN medium TEXT DEFAULT 'Offline'")
            conn.commit()
        ...
        # Add skill_points column if missing
        try:
            cursor.execute("SELECT skill_points FROM pet_status LIMIT 1")
        except sqlite3.OperationalError:
            try:
                cursor.execute("ALTER TABLE pet_status ADD COLUMN skill_points INTEGER DEFAULT 3")
                conn.commit()
            except Exception:
                pass

        # Verify pet_unlocked_skills table
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS pet_unlocked_skills (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        # Seed default side quests if empty
        cursor.execute("SELECT COUNT(*) FROM side_quests")
        if cursor.fetchone()[0] == 0:
            default_side_quests = [
                ("🧮 Trivia Master", "Win 2 Knowledge Guardian Trivia battles", "Trivia", 30, 1, 2, 0, 0),
                ("🗺️ Zone Crawler", "Complete 1 exploration dungeon room", "Dungeon", 25, 1, 1, 0, 0),
                ("🔥 Daily Discipline", "Complete 3 main daily quests", "Tasks", 40, 2, 3, 0, 0),
                ("💎 XP Collector", "Earn 50 total XP across missions & quests", "General", 35, 1, 50, 0, 0)
            ]
            cursor.executemany("""
                INSERT INTO side_quests (title, description, category, reward_xp, reward_sp, target_count, current_count, is_claimed)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, default_side_quests)
        conn.commit()
        ...
        # 5. Auto-seed & Recalibrate XP Store Items
        pet_store_items = [
            ("🥩 Cyber-Protein", 5, 10),
            ("💾 Memory Chip", 5, 10),
            ("🖌️ Chameleon Ink", 5, 10),
            ("⚡ Giga-Soda", 5, 10),
            ("🔮 Omni-Treat", 10, 5),
            ("🗝️ Evolution Matrix", 25, 2)
        ]

        real_world_rewards = [
            ("🎮 30 Mins Gaming / YouTube Time", 50, 10),
            ("🎮 1 Hour Gaming / YouTube Time", 100, 10),
            ("💵 $5 Cash Allowance", 250, 5),
            ("🪙 800 Minecoins (Minecraft)", 300, 5),
            ("🪨 1000 Shiny Rocks (Gorilla Tag)", 300, 5),
            ("📅 1 Day Off Studies", 750, 2),
            ("🕹️ New Game (Up to $20)", 1000, 1)
        ]
        
        for name, cost, qty in pet_store_items + real_world_rewards:
            cursor.execute("SELECT id FROM rewards WHERE name LIKE ?", (f"%{name.split()[-1]}%",))
            row = cursor.fetchone()
            if row:
                cursor.execute("UPDATE rewards SET xp_cost = ?, name = ? WHERE id = ?", (cost, name, row[0]))
            else:
        ...
        try:
            ...
            cursor.execute("INSERT OR IGNORE INTO app_config (key, value) VALUES ('note_min_length', '15')")
            conn.commit()
...
def complete_creator_project(project_id, summary="", attachment=""):
    ...
    try:
        ...
        # XP value is still stored in creator_projects.xp_reward and tracked via get_xp_balance().
        # Pet logic lives in: research_and_development/gamification/database_stubs.py
...
def add_task_to_db(title, category, video_url, xp_reward, target_date, is_boss, medium="Offline"):
    date_string = target_date.strftime("%Y-%m-%d")
    conn = sqlite3.connect('flokus.db')
    try:
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO tasks (title, category, task_date, video_url, xp_reward, is_boss_fight, medium) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (title, category, date_string, video_url, xp_reward, is_boss, medium))
        conn.commit()
    finally:
        conn.close()
...
def get_completed_tasks(view_date):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, title, category, video_url, xp_reward, is_boss_fight, task_summary, medium 
            FROM tasks 
...
def update_task_details(task_id, title, category, video_url, xp_reward, target_date, is_boss, medium="Offline"):
    date_string = target_date.strftime("%Y-%m-%d")
    conn = sqlite3.connect('flokus.db')
    try:
        cursor = conn.cursor()
        cursor.execute("""
            UPDATE tasks 
            SET title = ?, category = ?, video_url = ?, xp_reward = ?, task_date = ?, is_boss_fight = ?, medium = ?
            WHERE id = ?
        """, (title, category, video_url, xp_reward, date_string, is_boss, medium, task_id))
        conn.commit()
    finally:
        conn.close()
...
def get_all_pending_tasks():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, title, category, video_url, xp_reward, is_boss_fight, task_date, medium 
            FROM tasks 
...
# ==========================================
# DIGITAL PET LOGIC & ACTIONS — QUARANTINED
# Moved to: research_and_development/gamification/database_stubs.py
# Date: 2026-08-12 (Pre-launch stability decoupling)
# All pet tables remain intact in flokus.db.
# To reactivate: merge database_stubs.py back into this file.
# ==========================================

# The following functions have been quarantined:
#   calculate_evolution_internal()   → database_stubs.py
#   process_pet_xp_and_leveling()   → database_stubs.py
#   get_pet_status()                → database_stubs.py
#   use_pet_item()                  → database_stubs.py
#   get_pet_inventory()             → database_stubs.py
#   deduct_pet_stamina()            → database_stubs.py
#   complete_quest_room()           → database_stubs.py
#   fail_quest_room()               → database_stubs.py
#   _get_active_pet_id()            → database_stubs.py
#   get_pet_skill_points()          → database_stubs.py
#   get_unlocked_skills()           → database_stubs.py
#   unlock_pet_skill()              → database_stubs.py
#   override_pet_status()           → database_stubs.py
#   get_side_quests()               → database_stubs.py
#   claim_side_quest_reward()       → database_stubs.py
#   update_side_quest_progress()    → database_stubs.py
#   record_trivia_victory()         → database_stubs.py
